"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import { MessageCircle, X, Send, Smile } from "lucide-react";

type ChatMessage = {
  id: string;
  sessionId: string;
  text: string;
  from: "user" | "bot";
  createdAt: number;
};

const WELCOME: ChatMessage = {
  id: "welcome",
  sessionId: "",
  text: "Hello 👋 Welcome to Legacy Safety Solutions. How can we help you?",
  from: "bot",
  createdAt: 0,
};

function getSessionId(): string {
  const key = "legacy-chat-session";
  let id = localStorage.getItem(key);
  if (!id) {
    id = `ls-${Math.random().toString(36).slice(2, 10)}`;
    localStorage.setItem(key, id);
  }
  return id;
}

export default function ChatWidget() {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState<ChatMessage[]>([WELCOME]);
  const [input, setInput] = useState("");
  const [sending, setSending] = useState(false);
  const [sessionId, setSessionId] = useState("");
  const [activeTab, setActiveTab] = useState<"chat" | "whatsapp">("chat");
  const lastPollRef = useRef(0);
  const messagesEndRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    setSessionId(getSessionId());
  }, []);

  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]);

  const pollMessages = useCallback(async () => {
    if (!sessionId) return;

    try {
      const res = await fetch(
        `/api/chat/messages?sessionId=${encodeURIComponent(sessionId)}&since=${lastPollRef.current}`
      );
      const data = await res.json();

      if (data.messages?.length > 0) {
        setMessages((prev) => {
          const existingIds = new Set(prev.map((m) => m.id));
          const newMsgs = data.messages.filter(
            (m: ChatMessage) => !existingIds.has(m.id)
          );
          if (newMsgs.length === 0) return prev;
          return [...prev, ...newMsgs];
        });
        const latest = data.messages[data.messages.length - 1];
        lastPollRef.current = latest.createdAt;
      }
    } catch {
      // silent poll failure
    }
  }, [sessionId]);

  useEffect(() => {
    if (!open || !sessionId) return;

    pollMessages();
    const interval = setInterval(pollMessages, 2500);
    return () => clearInterval(interval);
  }, [open, sessionId, pollMessages]);

  async function handleSend(e: React.FormEvent) {
    e.preventDefault();
    const text = input.trim();
    if (!text || !sessionId || sending) return;

    setSending(true);
    setInput("");

    const optimistic: ChatMessage = {
      id: `temp-${Date.now()}`,
      sessionId,
      text,
      from: "user",
      createdAt: Date.now(),
    };
    setMessages((prev) => [...prev, optimistic]);

    try {
      const res = await fetch("/api/chat/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId, message: text }),
      });

      const data = await res.json();

      if (!res.ok) throw new Error(data.error);

      if (data.message) {
        setMessages((prev) =>
          prev.map((m) => (m.id === optimistic.id ? data.message : m))
        );
        lastPollRef.current = Math.max(
          lastPollRef.current,
          data.message.createdAt
        );
      }
    } catch {
      setMessages((prev) => prev.filter((m) => m.id !== optimistic.id));
      setInput(text);
    } finally {
      setSending(false);
    }
  }

  if (!open) {
    return (
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#ef6c00] text-white shadow-lg shadow-[#ef6c00]/40 transition-transform hover:scale-105"
        aria-label="Open chat"
      >
        <MessageCircle className="h-7 w-7" />
      </button>
    );
  }

  return (
    <div className="fixed bottom-6 right-6 z-50 flex w-[340px] flex-col overflow-hidden rounded-lg shadow-2xl sm:w-[360px]">
      <div className="flex items-center justify-between bg-[#ef6c00] px-4 py-3">
        <div>
          <p className="text-sm font-bold text-[#1a1a1a]">
            Legacy Safety Solutions
          </p>
          <div className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-green-500" />
            <span className="text-xs text-[#1a1a1a]/80">Online</span>
          </div>
        </div>
        <button
          type="button"
          onClick={() => setOpen(false)}
          className="rounded p-1 text-[#1a1a1a]/70 transition-colors hover:bg-black/10"
          aria-label="Close chat"
        >
          <X className="h-5 w-5" />
        </button>
      </div>

      {activeTab === "chat" ? (
        <>
          <div className="flex max-h-[320px] min-h-[280px] flex-col gap-3 overflow-y-auto bg-[#f5f5f5] px-3 py-4">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`max-w-[85%] rounded-lg px-3 py-2.5 text-sm leading-relaxed shadow-sm ${
                  msg.from === "user"
                    ? "ml-auto bg-[#ef6c00] text-white"
                    : "mr-auto bg-white text-gray-800"
                }`}
              >
                {msg.text}
              </div>
            ))}
            <div ref={messagesEndRef} />
          </div>

          <form onSubmit={handleSend} className="border-t border-gray-200 bg-white p-3">
            <div className="flex items-end gap-2">
              <textarea
                value={input}
                onChange={(e) => setInput(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !e.shiftKey) {
                    e.preventDefault();
                    handleSend(e);
                  }
                }}
                placeholder="Type your message..."
                rows={2}
                disabled={sending}
                className="flex-1 resize-none text-sm text-gray-800 outline-none placeholder:text-gray-400 disabled:opacity-50"
              />
              <div className="flex flex-col gap-1">
                <Smile className="h-4 w-4 text-gray-400" />
                <button
                  type="submit"
                  disabled={sending || !input.trim()}
                  className="text-[#ef6c00] disabled:opacity-40"
                  aria-label="Send message"
                >
                  <Send className="h-4 w-4" />
                </button>
              </div>
            </div>
          </form>
        </>
      ) : (
        <div className="flex min-h-[280px] flex-col items-center justify-center gap-4 bg-[#f5f5f5] p-6 text-center">
          <p className="text-sm text-gray-700">
            Contact us on WhatsApp for quick support.
          </p>
          <a
            href="https://wa.me/19377412543"
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-lg bg-[#25D366] px-6 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90"
          >
            Open WhatsApp
          </a>
        </div>
      )}

      <div className="flex border-t border-gray-200">
        <button
          type="button"
          onClick={() => setActiveTab("chat")}
          className={`flex flex-1 items-center justify-center py-3 transition-colors ${
            activeTab === "chat" ? "bg-white" : "bg-gray-100"
          }`}
        >
          <MessageCircle
            className={`h-6 w-6 ${activeTab === "chat" ? "text-[#ef6c00]" : "text-gray-500"}`}
          />
        </button>
        <button
          type="button"
          onClick={() => setActiveTab("whatsapp")}
          className={`flex flex-1 items-center justify-center py-3 transition-colors ${
            activeTab === "whatsapp" ? "bg-white" : "bg-gray-100"
          }`}
        >
          <svg
            viewBox="0 0 24 24"
            className={`h-6 w-6 ${activeTab === "whatsapp" ? "text-[#25D366]" : "text-gray-500"}`}
            fill="currentColor"
          >
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.435 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
        </button>
      </div>
    </div>
  );
}
